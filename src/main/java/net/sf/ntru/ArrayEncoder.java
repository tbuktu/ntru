...
public class ArrayEncoder {
     ...
     */
    static int[] decodeModQ(ByteBuffer buf, int N, int q) {
     ...
     * @return the decoded polynomial
     */
    static int[] decodeModQ(InputStream is, int N, int q) throws IOException {
        int qBits = 31 - Integer.numberOfLeadingZeros(q);
        int size = (N*qBits+7) / 8;
        byte[] arr = new byte[size];
        is.read(arr);
     ...
     */
    static int[] decodeMod3(byte[] data, int N) {
        int[] coeffs = new int[N];
        int coeffIndex = 0;
     ...
     * @return the encoded array
     */
    static byte[] encodeMod3(int[] arr) {
     ...
     * @return the encoded array
     */
    static byte[] encodeMod3Arith(int[] intArray) {
     ...
     */
    static int[] decodeMod3Arith(byte[] b, int N) {
     ...
     * @return the decoded array
     */
    static int[] decodeMod3Arith(ByteBuffer buf, int N) {
     ...
     * @return the decoded array
     */
    static int[] decodeMod3Arith(InputStream is, int N) throws IOException {
        int size = (int)Math.ceil(N * Math.log(3) / Math.log(2) / 8);
        byte[] arr = new byte[size];
        is.read(arr);
        return decodeMod3Arith(arr, N);
